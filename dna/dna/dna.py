...
def main():
    ...
    # Load STR counts and names from the CSV file
    with open(database_filename) as csvfile:
        reader = csv.DictReader(csvfile)
        str_names = reader.fieldnames[1:]  # Extract STR names from the header (excluding 'name')
        database = [row for row in reader]

    # Read DNA sequence file into a variable
    with open(dna_sequence_filename) as txtfile:
        sequence = txtfile.read().strip()
